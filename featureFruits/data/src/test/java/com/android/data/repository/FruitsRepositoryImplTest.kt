...
import com.android.common.model.Nutritions
...
/**
 * Unit tests for [FruitsRepositoryImpl].
 */
class FruitsRepositoryImplTest {
    ...
    fun `getNutrition returns success`() = runTest {
        ...
        coEvery { fruitsApi.getNutritions(fruitId.toString()) } returns Response.success(
            mockResponse
        )
    ...
    fun `getNutrition returns failure on error response`() = runTest {
        val fruitId = "1"
        coEvery { fruitsApi.getNutritions(fruitId) } returns Response.error(
            500,
            "".toResponseBody()
        )
